...
const OnboardingForm = ({ totalBalance, setCategoryBalance }) => {
  ...
  return (
    <form className="onboarding-form" onSubmit={handleSubmit}>
      ...
      <h2 className="onboarding-form__subheading">
        Allocate your account balance into each category as you see fit
      </h2>
      {moneyCategories.map((category, index) => {
        return (
          <div className="onboarding-form__list-item" key={index}>
            <label className="onboarding-form__label" htmlFor={category}>
              {category}
            </label>
            {inputErrors[category] && (
              <span className="onboarding-form__required-text">Required</span>
            )}
            <input
              className={`onboarding-form__input ${
                !isFormValid &&
                inputErrors[category] &&
                "onboarding-form__input--error"
              }`}
              type="text"
              id={category}
              name={category}
      ...
      <div className="onboarding-form__balance">
        ...
        <h4
          className={`onboarding-form__balance-amount ${
            (balanceError ||
              Object.keys(inputErrors).some(
                (category) => inputErrors[category]
              )) &&
            "onboarding-form__balance-error"
          }`}
        >
          ${remainingBalance}
        </h4>
      ...
      <div className="onboarding-form__error-container">
        {!isFormValid &&
          (Object.keys(inputErrors).length > 0 || balanceError) && (
            <div className="onboarding-form__balance-error-text">
              Please allocate your remaining balance
            </div>
          )}
      </div>
